...
class Support:
    ...
    def __init__(self, xstr_min=0.1, xstr_max=2, buff_len=10, k=8, dt=0.001, g=0, Nt=1000):

        """
        Construct a supported dimension (til_f) spanned with buff_len tuning curves from xstr_min to xstr_max.


        Keyword arguments:

        xstr_min -- smallest xstar (lowest value that til_f can encode)
        xstr_max -- largest xstar (highest value that til_f can encode)
        buff_len -- number of tuning curves that consitute til_f
        k -- value of k (determines sharpnes of the tuning curves)
        dt -- time step
        g -- parameter g (determinizes scaling of til_f - g=1->equal amplitude, g=0->power-law decay of amplitude)
        Nt -- number of (time) points
        """

        self.xstr_min = xstr_min
        self.xstr_max = xstr_max
        ...
        self.N = self.buff_len+2*self.k  # Total number of leaky integrators (2k explansion needed to compute the derivate)

        # Create power-law growing xstarlist and compute corresponding s
        a = (self.xstr_max/self.xstr_min)**(1./buff_len)-1
        pow_vec = np.arange(-self.k, buff_len + self.k)
        self.xstarlist = self.xstr_min * (1 + a)**pow_vec
        s = self.k/self.xstarlist
    ...
    def invert(self, time_index):
        ...
        L2 = (F_diff/factorial(self.k))*(self.xstarlist**self.g)
    ...
    # Encode a scaler as a distribution
